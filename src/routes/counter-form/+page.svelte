...
<main>
	...
	<p class="description">
		This page demonstrates the <code>form</code> remote function. Forms work without JavaScript and automatically
		enhance with progressive enhancement for better UX.
	</p>
	...
	{#if incrementCounterForm.result || resetCounterForm.result}
		<div class="results">
			{#if incrementCounterForm.result}
				<div
					class="result"
					class:success={incrementCounterForm.result.success}
					class:error={!incrementCounterForm.result.success}
				>
					{incrementCounterForm.result.success
						? incrementCounterForm.result.message
						: incrementCounterForm.result.error}
				</div>
			{/if}
